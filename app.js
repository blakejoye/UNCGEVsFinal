...
app.post('/api/maintenance', async (req, res) => {
    ...
    try {
        const result = await pool.query(
            "INSERT INTO maintenance_reports (chargerid, reported_by, issue_description, status) VALUES ($1, $2, $3, 'pending') RETURNING *",
...
// ==================== Maintenance Routes ====================app.get('/maintenance', async (req, res) => {
app.get('/maintenance', async (req, res) => {
    try {
        const result = await pool.query(`
            SELECT m.*, c.location, u.username AS reported_by_name
            FROM maintenance_reports m
                     LEFT JOIN chargers c ON m.chargerid = c.chargerid
...
app.put('/maintenance/:id/assign', async (req, res) => {
    ...
    try {
        ...
        if (result.rows.length === 0) return res.status(404).json({ message: "Report not found" });
        res.json({ message: "Ticket assigned successfully", report: result.rows[0] });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
...
app.post('/signup', async (req, res) => {
    ...
    if (!email.endsWith("@uncg.edu")) {
        return res.status(403).json({ success: false, message: "Only UNCG emails are allowed." });
    }
...
app.post('/login', async (req, res) => {
    ...
    try {
        ...
        res.json({ success: true, redirect: "/welcome.html", userId: user.id, username: user.name });
    } catch (err) {
...
// Route to get only pending maintenance reports
app.get('/api/maintenance', async (req, res) => {
    try {
        ...
        console.error("Error fetching pending maintenance reports:", err);
